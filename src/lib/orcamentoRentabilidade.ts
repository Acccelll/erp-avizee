...
export interface RentabilidadeItem {
  ...
  observacaoInternaCenario?: string;
...
export function calcularRentabilidade(
  ...
  const analysisItems = validItems.map(({ item, originalIndex }): RentabilidadeItem => {
    ...
    const freteScenarioOverride = item.frete_rateado_simulado_unitario != null ? Math.max(0, safeNumber(item.frete_rateado_simulado_unitario)) : freteScenario;
    const impostoScenarioOverride = item.imposto_rateado_simulado_unitario != null ? Math.max(0, safeNumber(item.imposto_rateado_simulado_unitario)) : impostoScenario;
    ...
    const custoFinalScenario = custoScenario == null ? null : round2(custoScenario + freteScenarioOverride + impostoScenarioOverride + outrosScenario);
    ...
    return {
      ...
      freteRateadoSimulado: useScenario ? freteScenarioOverride : freteBase,
      impostoRateadoBase: impostoBase,
      impostoRateadoSimulado: useScenario ? impostoScenarioOverride : impostoBase,
      ...
      observacaoInternaCenario: item.observacao_interna_cenario || "",
  ...
  const itensAlterados = analysisItems.filter((item) => item.usarCenario).length;
