  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const merged: EntityResult[] = [
        ...(clientes.data || []).map((c: any) => ({ id: `cli-${c.id}`, title: c.nome_razao_social, subtitle: c.cpf_cnpj ? `CPF/CNPJ: ${c.cpf_cnpj}` : 'Cliente', path: '/clientes', category: 'Clientes' as const })),
